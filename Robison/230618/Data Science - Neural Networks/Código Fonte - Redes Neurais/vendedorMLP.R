...
fitModelNeuralNetworkMLP <- function(training.set)
{
  ...
  fit.neural.network = neuralnet(formula,
                                 data=training.set,      # atribuido dataset para uso
                                 linear.output=TRUE,     # Segue uma estrutura de regress�o
                                 hidden=c(3,2),          # Camada Oculta
                                 threshold =0.01,        # POndera��o peso
                                 rep=50,                 # Volume das Epocas (ciclos de execu��o)
